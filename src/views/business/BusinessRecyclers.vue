...
<template>
  ...
  >
    <div class="space-y-6">
      <!-- Header -->
      <div>
        <h1 class="text-[24px] font-bold text-gray-900">{{ $t('businessRecyclers.title') }}</h1>
        <p class="text-[16px] text-gray-600 mt-1">{{ $t('businessRecyclers.subtitle') }}</p>
      </div>
      ...
      <div class="bg-gradient-to-r from-emerald-600 to-teal-600 rounded-2xl p-6 text-white">
        <div class="flex items-start justify-between">
          <div class="flex items-start gap-4">
            ...
            <div>
              <h2 class="text-[22px] font-bold">{{ $t('businessRecyclers.catalogTitle') }}</h2>
              <p class="text-[16px] text-emerald-100 mt-1 max-w-2xl">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mt-6">
          <div class="bg-white/10 rounded-xl p-4">
            ...
            <p class="text-emerald-100 text-[14px]">{{ $t('businessRecyclers.statRecyclers') }}</p>
          </div>
          <div class="bg-white/10 rounded-xl p-4">
            <p class="text-3xl font-bold">{{ stats.partners }}</p>
            <p class="text-emerald-100 text-[14px]">{{ $t('businessRecyclers.statPartners') }}</p>
          </div>
          <div class="bg-white/10 rounded-xl p-4">
            <p class="text-3xl font-bold">{{ (stats.totalCapacity / 1000).toFixed(0) }}K</p>
            <p class="text-emerald-100 text-[14px]">{{ $t('businessRecyclers.statCapacity') }}</p>
          </div>
          <div class="bg-white/10 rounded-xl p-4">
            <p class="text-3xl font-bold">{{ stats.regions }}</p>
            <p class="text-emerald-100 text-[14px]">{{ $t('businessRecyclers.statRegions') }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
        ...
        <div class="flex items-center justify-end mt-4 pt-4 border-t border-gray-200">
          <div class="flex items-center gap-2">
            <span class="text-[14px] text-gray-500">{{ $t('businessRecyclers.viewLabel') }}</span>
            <button
              @click="viewMode = 'grid'"
              :class="[
      ...
      <div v-if="viewMode === 'grid'" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        >
          ...
          <div class="p-4 border-b border-gray-100">
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1 min-w-0">
                <h3 class="text-[18px] font-semibold text-gray-900 truncate">{{ recycler.name }}</h3>
                <p class="text-[14px] text-gray-500">{{ recycler.region }}</p>
              </div>
            </div>
          </div>
          ...
          <!-- Content -->
          <div class="p-4">
            <p class="text-[14px] text-gray-600 line-clamp-2 mb-3">{{ recycler.address }}</p>
            ...
            <div class="flex flex-wrap gap-1 mb-3">
              <span
                ...
                :key="wt"
                class="px-2 py-0.5 bg-gray-100 text-gray-700 text-[14px] rounded-full"
              >
                {{ getGroupShortLabel(wt) }}
              </span>
              <span v-if="recycler.wasteTypes.length > 3" class="px-2 py-0.5 bg-gray-100 text-gray-700 text-[14px] rounded-full">+{{ recycler.wasteTypes.length - 3 }}</span>
            </div>
            ...
            <div class="flex items-center gap-2 mb-3">
              ...
              <span class="text-[14px] font-medium text-gray-700">{{ recycler.rating }}</span>
            </div>
          ...
          <div class="px-4 py-3 bg-gray-50 border-t border-gray-100 flex gap-2">
            <button
              ...
              class="flex-1 px-3 py-2 text-[14px] font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors"
            >
              {{ $t('businessRecyclers.details') }}
            </button>
            <button
              @click="openRequestModal(recycler)"
              class="flex-1 px-3 py-2 text-[14px] font-medium text-white bg-emerald-600 rounded-lg hover:bg-emerald-700 transition-colors"
            >
          ...
          <div class="flex items-start gap-4">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-start justify-between gap-4">
                <div>
                  <h3 class="text-[18px] font-semibold text-gray-900">{{ recycler.name }}</h3>
                  <p class="text-[14px] text-gray-500 mt-0.5">{{ recycler.region }} · {{ recycler.address }}</p>
                  <p class="text-[14px] text-gray-600 mt-2">{{ recycler.fullName }}</p>
                  <div class="flex flex-wrap items-center gap-4 mt-3">
                    ...
                    <div class="flex items-center gap-1">
                      ...
                      <span class="text-[14px] font-medium">{{ recycler.rating }}</span>
                    </div>
                    <span class="text-[14px] text-gray-500">{{ $t('businessRecyclers.capacity') }} {{ recyclerStore.getTotalCapacity(recycler).toLocaleString() }} {{ $t('businessRecyclers.tonsPerYear') }}</span>
                  </div>
                </div>
                <div class="flex gap-2 flex-shrink-0">
                  <button
                    @click="openDetails(recycler)"
                    class="px-4 py-2 text-[14px] font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors"
                  >
                    {{ $t('businessRecyclers.details') }}
                  </button>
                  <button
                    @click="openRequestModal(recycler)"
                    class="px-4 py-2 text-[14px] font-medium text-white bg-emerald-600 rounded-lg hover:bg-emerald-700 transition-colors"
                  >
      ...
      <div v-if="filteredRecyclers.length === 0" class="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
        ...
        <h3 class="text-[22px] font-medium text-gray-900 mb-1">{{ $t('businessRecyclers.emptyTitle') }}</h3>
        <p class="text-[16px] text-gray-500">{{ $t('businessRecyclers.emptyDesc') }}</p>
      </div>
    </div>
      ...
      <div v-if="showDetailsModal && selectedRecycler" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-2xl shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
          <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between sticky top-0 bg-white">
            <h3 class="text-[22px] font-semibold text-gray-900">{{ $t('businessRecyclers.companyInfo') }}</h3>
          ...
          <div class="p-6 space-y-6">
            ...
            <div class="flex items-start gap-4">
              ...
              <div class="flex-1">
                <h4 class="text-[22px] font-bold text-gray-900">{{ selectedRecycler.name }}</h4>
                <p class="text-[16px] text-gray-500 mt-1">{{ selectedRecycler.fullName }}</p>
                <div class="flex items-center gap-3 mt-2">
                  <div class="flex items-center gap-1">
                    <svg class="w-4 h-4 text-yellow-400" fill="currentColor" viewBox="0 0 20 20">
                      <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                    </svg>
                    <span class="text-[16px] font-medium">{{ selectedRecycler.rating }}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div class="bg-gray-50 rounded-xl p-4">
                <h5 class="text-[14px] font-medium text-gray-500 mb-3">{{ $t('businessRecyclers.contacts') }}</h5>
                <div class="space-y-2">
                  <div class="flex items-center gap-2">
                    ...
                    <span class="text-[14px] text-gray-700">{{ selectedRecycler.address }}</span>
                  </div>
                  <div class="flex items-center gap-2">
                    <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                    </svg>
                    <span class="text-[14px] text-gray-700">{{ selectedRecycler.contactPhone }}</span>
                  </div>
                  <div class="flex items-center gap-2">
                    <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                    </svg>
                    <span class="text-[14px] text-gray-700">{{ selectedRecycler.contactEmail }}</span>
                  </div>
                  <div class="flex items-center gap-2">
                    <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" />
                    </svg>
                    <span class="text-[14px] text-gray-700">{{ selectedRecycler.website }}</span>
                  </div>
                </div>
              </div>

              <div class="bg-gray-50 rounded-xl p-4">
                <h5 class="text-[14px] font-medium text-gray-500 mb-3">{{ $t('businessRecyclers.aboutCompany') }}</h5>
                <div class="space-y-2">
                  <div class="flex justify-between">
                    <span class="text-[14px] text-gray-500">{{ $t('businessRecyclers.employees') }}</span>
                    <span class="text-[14px] font-medium text-gray-700">{{ selectedRecycler.employeesCount }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-[14px] text-gray-500">{{ $t('businessRecyclers.capacityLabel') }}</span>
                    <span class="text-[14px] font-medium text-gray-700">{{ recyclerStore.getTotalCapacity(selectedRecycler).toLocaleString() }} {{ $t('businessRecyclers.tonsPerYear') }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-[14px] text-gray-500">{{ $t('businessRecyclers.regionLabel') }}</span>
                    <span class="text-[14px] font-medium text-gray-700">{{ selectedRecycler.region }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-[14px] text-gray-500">{{ $t('businessRecyclers.innLabel') }}</span>
                    <span class="text-[14px] font-medium text-gray-700">{{ selectedRecycler.inn }}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <!-- Waste types -->
            <div>
              <h5 class="text-[14px] font-medium text-gray-500 mb-3">{{ $t('businessRecyclers.acceptedWasteTypes') }}</h5>
              <div class="flex flex-wrap gap-2">
                <span
                  v-for="wt in selectedRecycler.wasteTypes"
                  :key="wt"
                  class="px-3 py-1.5 bg-emerald-100 text-emerald-700 text-[14px] font-medium rounded-lg"
                >
                  {{ getGroupShortLabel(wt) }}
                </span>
              </div>
            ...
            <!-- Processing methods -->
            <div>
              <h5 class="text-[14px] font-medium text-gray-500 mb-3">{{ $t('businessRecyclers.processingMethods') }}</h5>
              <div class="flex flex-wrap gap-2">
                <span
                  v-for="method in selectedRecycler.processingMethods"
                  :key="method"
                  class="px-3 py-1.5 bg-blue-100 text-blue-700 text-[14px] font-medium rounded-lg"
                >
                  {{ method }}
                </span>
              </div>
            ...
            <div v-if="selectedRecycler.certifications.length > 0">
              <h5 class="text-[14px] font-medium text-gray-500 mb-3">{{ $t('businessRecyclers.certificates') }}</h5>
              <div class="flex flex-wrap gap-2">
                <span
                  v-for="cert in selectedRecycler.certifications"
                  :key="cert"
                  class="px-3 py-1.5 bg-amber-100 text-amber-700 text-[14px] font-medium rounded-lg flex items-center gap-1"
                >
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="flex gap-3 pt-4 border-t border-gray-200">
              <button
                ...
                class="flex-1 px-4 py-2.5 text-[14px] bg-emerald-600 text-white rounded-lg font-medium hover:bg-emerald-700 transition-colors"
              >
                {{ $t('businessRecyclers.sendRequest') }}
              </button>
              <button
                @click="closeDetails"
                class="flex-1 px-4 py-2.5 text-[14px] bg-gray-100 text-gray-700 rounded-lg font-medium hover:bg-gray-200 transition-colors"
              >
                {{ $t('common.close') }}
              </button>
            </div>
      ...
      <div v-if="showRequestModal && selectedRecycler" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-2xl shadow-xl max-w-lg w-full">
          <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            <div>
              <h3 class="text-[22px] font-semibold text-gray-900">{{ $t('businessRecyclers.requestTitle') }}</h3>
              <p class="text-[14px] text-gray-500">{{ selectedRecycler.name }}</p>
            </div>
          ...
          <div class="p-6 space-y-4">
            <div>
              <label class="block text-[14px] font-medium text-gray-700 mb-1">{{ $t('businessRecyclers.wasteTypeLabel') }}</label>
              <Select
            ...
            <div>
              <label class="block text-[14px] font-medium text-gray-700 mb-1">{{ $t('businessRecyclers.approxVolume') }}</label>
              <input
            ...
            <div>
              <label class="block text-[14px] font-medium text-gray-700 mb-1">{{ $t('businessRecyclers.frequency') }}</label>
              <div class="flex gap-3">
                <label class="flex items-center gap-2 cursor-pointer">
                  <input type="radio" v-model="requestForm.frequency" value="once" class="text-emerald-600" />
                  <span class="text-[14px] text-gray-700">{{ $t('businessRecyclers.frequencyOnce') }}</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer">
                  <input type="radio" v-model="requestForm.frequency" value="regular" class="text-emerald-600" />
                  <span class="text-[14px] text-gray-700">{{ $t('businessRecyclers.frequencyRegular') }}</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer">
                  <input type="radio" v-model="requestForm.frequency" value="contract" class="text-emerald-600" />
                  <span class="text-[14px] text-gray-700">{{ $t('businessRecyclers.frequencyContract') }}</span>
                </label>
              </div>
            </div>

            <div>
              <label class="block text-[14px] font-medium text-gray-700 mb-1">{{ $t('businessRecyclers.comment') }}</label>
              <textarea
          ...
          <div class="px-6 py-4 border-t border-gray-200 flex gap-3">
            <button
              ...
              class="flex-1 px-4 py-2 text-[14px] text-gray-700 bg-gray-100 rounded-lg font-medium hover:bg-gray-200 transition-colors"
            >
              {{ $t('common.cancel') }}
            </button>
            <button
              ...
              class="flex-1 px-4 py-2 text-[14px] bg-emerald-600 text-white rounded-lg font-medium hover:bg-emerald-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {{ $t('businessRecyclers.sendRequest') }}
            </button>
          </div>
