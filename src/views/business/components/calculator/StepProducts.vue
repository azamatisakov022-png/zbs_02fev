...
<template>
  <div class="p-6 lg:p-8">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
      <h2 class="text-[27px] font-semibold text-slate-800">{{ $t('businessCalc.productsTitle') }}</h2>
      <button @click="emit('import-declaration')" class="flex items-center gap-2 text-[20px] font-semibold text-amber-500 hover:bg-amber-50 px-4 py-2 rounded-lg transition-colors">
    ...
    <button @click="emit('add-item')" class="mt-4 flex items-center gap-2 text-[20px] font-semibold text-amber-500 hover:bg-amber-50 px-4 py-2 rounded-lg transition-colors">
      <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<style scoped>
.cf-total-banner {
  background: white; border: 1px solid var(--color-slate-200); border-radius: 16px;
  padding: 20px 24px; box-shadow: var(--shadow-sm);
}
.cf-total-banner__grid {
  display: grid; grid-template-columns: repeat(5, 1fr); gap: 12px;
  margin-bottom: 16px; padding-bottom: 16px; border-bottom: 1px solid var(--color-slate-200);
}
.cf-total-banner__cell { display: flex; flex-direction: column; gap: 4px; }
.cf-total-banner__cell-label {
  font-size: 15px; color: var(--color-slate-800); font-weight: 600;
  text-transform: uppercase; letter-spacing: 0.3px;
}
.cf-total-banner__cell-value { font-size: 20px; font-weight: 700; color: var(--color-slate-800); }
.cf-total-banner__bottom {
  display: flex; align-items: center; justify-content: space-between;
  flex-wrap: wrap; gap: 12px;
}
.cf-total-banner__label { font-size: 22px; font-weight: 600; color: var(--color-slate-800); }
.cf-total-banner__value { font-size: 32px; font-weight: 800; }
