...
<template>
  ...
  >
    <div class="space-y-6">
      ...
      <AppCard radius="sm" padding="sm">
        <div class="flex flex-col md:flex-row gap-4">
          <div class="flex-1">
            <AppInput v-model="searchQuery" :placeholder="$t('common.search')" hideLabel focusColor="#14b8a6">
              <template #prefix>
      ...
      <AppCard v-if="filteredWasteTypes.length === 0" radius="sm" padding="lg">
    ...
    <!-- Details Modal -->
    <AppModal :visible="showDetailsModal && !!selectedWasteType" :title="$t('employeeWasteTypes.wasteTypeInfo')" size="lg" @close="showDetailsModal = false">
      ...
      <template #footer>
        <AppButton variant="primary" fullWidth bg="#0D9488" @click="toastStore.show({ type: 'info', title: $t('employeeWasteTypes.editToastTitle'), message: $t('employeeWasteTypes.editToastMessage') })">
          {{ $t('common.edit') }}
        </AppButton>
        <AppButton variant="secondary" fullWidth @click="showDetailsModal = false">
          {{ $t('common.close') }}
        </AppButton>
      </template>
    </AppModal>
