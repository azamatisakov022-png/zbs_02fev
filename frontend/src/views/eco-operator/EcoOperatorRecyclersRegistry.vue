...
<template>
  ...
  >
    ...
    <div class="flex flex-wrap gap-2 mb-4">
      ...
      <button
        @click="filterLicense = 'expired'"
        :class="[
          'inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-medium transition-all border',
          filterLicense === 'expired'
      ...
      >
        ...
        Истёкшие
        <span :class="['text-xs px-1.5 py-0.5 rounded-full', filterLicense === 'expired' ? 'bg-red-700 text-red-100' : 'bg-red-100 text-red-700']">{{ expiredTotalCount }}</span>
      </button>
    </div>
