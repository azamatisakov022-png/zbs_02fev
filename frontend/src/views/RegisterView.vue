...
const esiInnInput = ref('')
const esiError = ref('')

const handleEsiLogin = async () => {
  const inn = esiInnInput.value.trim()
  if (!inn || inn.length < 14) {
    esiError.value = t('register.errors.enterValidInn')
    return
  }
  esiError.value = ''
  ...
  <div class="bg-[#f8fafc] py-8 lg:py-12">
    <div class="container-main">
      ...
      <div v-if="registrationMode === 'choose' && !isEsiLoading" class="max-w-lg mx-auto py-8">
        ...
        <!-- INN Verification -->
        <div class="bg-white rounded-2xl p-6 border border-[#e2e8f0] space-y-4 mb-4">
          ...
          <input
            ...
            @keyup.enter="handleEsiLogin"
          />
          <p v-if="esiError" class="text-sm text-red-600">{{ esiError }}</p>
          <button
