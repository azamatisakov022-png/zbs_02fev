 ...
 *
 * По ТЗ (формулировка из контракта и FAQ):
 *   - производители товаров - ежеквартально, не позднее 15 числа месяца,
 *     следующего за отчётным кварталом;
 *   - импортёры товаров - по факту ввоза, не позднее 15 рабочих дней
 *     с момента ввоза.
 *
 * Пеня начисляется только ПОСЛЕ dueDate. До dueDate штрафа нет
 * (даже если сегодня уже больше даты ввоза).
 */
public final class DueDateCalculator {
    ...
    /**
     * Импортёр: дата ввоза + 15 рабочих дней (Сб/Вс пропускаются).
     */
    public static LocalDate forImporter(LocalDate importDate) {
        if (importDate == null) return null;
        return addWorkingDays(importDate, 15);
    }
